...
class DeliveryCaptainService {
  ...
  Future<void> loadCaptains({
    ...
    try {
      final queryBuilder = SupabaseService.client
      ...
      final response = await queryBuilder
          ...
          .limit(200);
      _currentCaptains = List<Map<String, dynamic>>.from(
        response,
      ).map((m) => DeliveryCaptain.fromMap(m)).toList();
  ...
  RealtimeChannel subscribeToCaptainsChanges() {
    final channel = SupabaseService.client.channel('public:delivery_captains');
    channel.on(
      RealtimeListenTypes.postgresChanges,
      ChannelFilter(event: '*', schema: 'public', table: 'delivery_captains'),
      (payload, [ref]) {
        // Refresh on any change (fire-and-forget - keep callback synchronous)
        loadCaptains();
      },
    );
    channel.subscribe();
