...
class CartService {
  ...
  void _subscribeToCartsChanges() {
    try {
      ...
      _cartsChannel = SupabaseService.client
    ...
    try {
      ...
      try {
        final rpcResult = await SupabaseService.client
            .rpc('accept_cart', params: {'p_cart_id': cartId});
        final bool rpcSuccess = rpcResult == true ||
            rpcResult == 1 ||
            (rpcResult is Map && (rpcResult['success'] == true));
        if (rpcSuccess) {
          // Update local state lists
          final cartIndex =
              _currentAvailableCarts.indexWhere((cart) => cart.id == cartId);
          if (cartIndex != -1) {
  ...
  Future<bool> updateCartStatus(String cartId, CartStatus status) async {
    ...
    try {
      ...
      if (response.isNotEmpty) {
        ...
        final cartIndex =
            _currentMyCarts.indexWhere((cart) => cart.id == cartId);
        if (cartIndex != -1) {
  ...
  Future<Cart?> getCartById(String cartId) async {
    try {
      final response = await SupabaseService.client.from('carts').select('''
            *,
            cart_items (
              *,
              products (
